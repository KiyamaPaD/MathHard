type CelebrationKind = "problem" | "lesson" | "exam" | "achievement" | "level" | "generic";
...
type MotionConfig = {
  ...
  confetti: Record<CelebrationKind, number>;
  selectors: {
    reveal: string[];
    progress: string[];
  };
...
async function loadConfig(): Promise<MotionConfig> {
  try {
    const response = await fetch("/data/microinteractions.json?v=4h", { cache: "force-cache" });
    if (!response.ok) return DEFAULT_CONFIG;
    const input = await response.json() as Partial<MotionConfig>;
    return {
...
class MhXpPulse extends HTMLElement {
  ...
  pulse(delta: number): void {
    ...
    if (this.valueNode) this.valueNode.textContent = `+${Math.round(delta)}`;
    if (this.labelNode) this.labelNode.textContent = `+${Math.round(delta)} XP`;
...
class ConfettiCanvas {
  ...
  burst(count: number, origin?: { x: number; y: number }): void {
...
export type MicrointeractionEngine = {
  celebrate(detail: CelebrationDetail): void;
  burst(kind?: CelebrationKind, origin?: { x: number; y: number }): void;
  animateElement(element: Element): void;
  destroy(): void;
};

export async function createMicrointeractionEngine(): Promise<MicrointeractionEngine> {
  ...
  let confetti: ConfettiCanvas | null = null;
  try {
  ...
  const counterValues = new WeakMap<Element, number>();
  const progressValues = new WeakMap<Element, string>();
  const activeCounterAnimations = new WeakSet<Element>();
  const revealSelector = config.selectors.reveal.join(",");
  const progressSelector = config.selectors.progress.join(",");
  let xpPulse: MhXpPulse | null = null;

  function ensureXpPulse(): MhXpPulse | null {
    if (xpPulse?.isConnected) return xpPulse;
    const xp = document.querySelector("#xpTotalHeader");
    const counter = xp?.closest(".counter");
    if (!counter) return null;
    xpPulse = document.createElement("mh-xp-pulse") as MhXpPulse;
    counter.append(xpPulse);
    return xpPulse;
  }

  function animateElement(element: Element): void {
    if (reduced || seen.has(element) || !(element instanceof HTMLElement)) return;
    if (element.closest("#math-loader, .modal[aria-hidden='true']")) return;
    seen.add(element);
  ...
  function animateNumber(element: Element): void {
    if (!(element instanceof HTMLElement)) return;
    if (activeCounterAnimations.has(element)) return;
    const target = safeNumber(element.textContent);
    if (target === null) return;
    const previous = counterValues.get(element);
    counterValues.set(element, target);
    if (previous === undefined || previous === target || reduced) return;
    ...
    if (element.id === "xpTotalHeader" && delta > 0) ensureXpPulse()?.pulse(delta);
  }

  function animateProgress(element: Element): void {
    if (!(element instanceof HTMLElement)) return;
    ...
    if (reduced || previous === current) return;
    element.animate(
      [
  ...
  function scan(root: ParentNode): void {
    if (root instanceof Element) {
      if (root.matches(revealSelector)) animateElement(root);
      if (root.matches(COUNTER_SELECTOR)) animateNumber(root);
      if (root.matches(progressSelector)) animateProgress(root);
    }
  ...
  document.querySelectorAll(COUNTER_SELECTOR).forEach((element) => {
    ...
    if (value !== null) counterValues.set(element, value);
  });
  document.querySelectorAll(progressSelector).forEach((element) => progressValues.set(element, (element as HTMLElement).style.width));
  ensureXpPulse();
  ...
  const mutationObserver = new MutationObserver((records) => {
    for (const record of records) {
      if (record.type === "characterData") {
        ...
        if (parent?.matches(COUNTER_SELECTOR)) animateNumber(parent);
        continue;
      }
      for (const node of record.addedNodes) {
        if (!(node instanceof Element)) continue;
        node.querySelectorAll?.(revealSelector).forEach((element) => intersectionObserver.observe(element));
        if (node.matches(revealSelector)) intersectionObserver.observe(node);
        scan(node);
      }
      if (record.target instanceof Element) {
        if (record.target.matches(COUNTER_SELECTOR)) animateNumber(record.target);
        if (record.target.matches(progressSelector)) animateProgress(record.target);
      }
    }
  });
  ...
  function createRipple(event: PointerEvent): void {
    ...
    if (!(target instanceof HTMLElement) || target.matches(":disabled") || target.closest("[aria-disabled='true']")) return;
  ...
  function onPointerUp(event: PointerEvent): void {
    const target = (event.target as Element | null)?.closest(INTERACTIVE_SELECTOR);
    if (!(target instanceof HTMLElement) || reduced) return;
    target.animate(
      [{ transform: "scale(.985)" }, { transform: "scale(1)" }],
      { duration: config.durations.tap, easing: "cubic-bezier(.2,.8,.2,1)" }
    );
  }

  function burst(kind: CelebrationKind = "generic", origin?: { x: number; y: number }): void {
    confetti?.burst(config.confetti[kind] ?? config.confetti.generic, origin);
  }

  function celebrate(detail: CelebrationDetail): void {
  ...
  function onCelebrate(event: Event): void {
    celebrate((event as CustomEvent<CelebrationDetail>).detail || {});
  }
