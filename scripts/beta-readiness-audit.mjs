...
for (const page of ["index.html", "profile.html", "u.html"]) {
  ...
  requireText(page, /mathhard-build[^>]+(?:4g4|4h|4i|4j|4k1|5a1|5a4)/, `${page} must expose the current beta-or-later build label.`);
}
...
requireText("js/runtime-diagnostics.js", /phase-5a4-deploy-synchronization/, "Runtime diagnostics build label is stale.");
