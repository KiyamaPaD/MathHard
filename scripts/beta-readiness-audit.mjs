...
requireText("profile.html", /beta-readiness-controller\.js\?v=4g4/, "Profile must load beta readiness controller.");
requireText("u.html", /beta-readiness-controller\.js\?v=4g4/, "Public profile must load beta readiness controller.");
...
requireText("js/runtime-diagnostics.js", /phase-4g4-beta-readiness/, "Runtime diagnostics build label is stale.");
...
for (const page of ["404.html", "offline.html"]) {
  ...
  if (!/system-page\.js\?v=4g4/.test(source)) errors.push(`${page} must load system-page.js.`);
}
