(function(){
  ...
  const T={
    ro:{title:'Explorer — axa numerelor reale',input:'Număr / expresie',add:'Adaugă',clear:'Șterge punctele',order:'Ordine',interval:'Interval',left:'Stânga',right:'Dreapta',closed:'inclus',unbounded:'nemărginit',hint:'Adaugă numere din câmpul de mai sus • trage markerul pentru a-l muta',bad:'Expresie neacceptată',empty:'Adaugă cel puțin un punct.',approx:'aprox.'},
    en:{title:'Real-number line explorer',input:'Number / expression',add:'Add',clear:'Clear points',order:'Order',interval:'Interval',left:'Left',right:'Right',closed:'closed',unbounded:'unbounded',hint:'Add numbers above • drag a marker to move it',bad:'Unsupported expression',empty:'Add at least one point.',approx:'approx.'}
  };
  ...
  function mount(id,host,options={}){
    ...
    root.innerHTML=`<style>
      ...
      .mh-real-nline__stage{position:relative;height:270px;border:1px dashed var(--border);border-radius:14px;overflow:hidden;background:color-mix(in srgb,var(--bg) 72%,var(--card) 28%);touch-action:none}
      ...
      .mh-real-nline__interval-wrap{border-top:1px solid var(--border);padding-top:10px;display:grid;gap:8px}.mh-real-nline__interval{display:none;gap:8px;align-items:center;flex-wrap:wrap}.mh-real-nline__interval-wrap[data-enabled="1"] .mh-real-nline__interval{display:flex}.mh-real-nline__interval label{display:inline-flex;align-items:center;gap:5px;font-size:.82rem;color:var(--muted)}
      @media(max-width:640px){.mh-real-nline__stage{height:300px}.mh-real-nline input[type=text]{max-width:150px}}
    </style>
    ...
    const state={points:[],nextId:1,dragId:null,viewMin:-5,viewMax:5,interval:{enabled:false,left:null,right:null,leftUnbounded:false,rightUnbounded:false,leftClosed:true,rightClosed:false}};
    ...
    function parseInterval(){
      ...
      state.interval={enabled,left:l,right:r,leftUnbounded:lu,rightUnbounded:ru,leftClosed:root.querySelector('.mh-rnl-left-closed').checked,rightClosed:root.querySelector('.mh-rnl-right-closed').checked};
      return !enabled || ((lu||l)&&(ru||r));
    }
    function fit(){
      const vals=state.points.map(p=>p.approxValue); const I=state.interval; if(I.enabled&&I.left) vals.push(I.left.approxValue); if(I.enabled&&I.right) vals.push(I.right.approxValue);
      if(!vals.length){state.viewMin=-5;state.viewMax=5;return;} let lo=Math.min(...vals),hi=Math.max(...vals); if(lo===hi){lo-=2;hi+=2;} const span=Math.max(2,hi-lo),m=span*.22; state.viewMin=(I.enabled&&I.leftUnbounded)?Math.min(lo-m,-5):lo-m; state.viewMax=(I.enabled&&I.rightUnbounded)?Math.max(hi+m,5):hi+m;
    }
    function draw(){
      ...
      const I=state.interval; if(I.enabled&&(I.leftUnbounded||I.left)&&(I.rightUnbounded||I.right)){
      ...
      const positioned=[...groups.values()].map(g=>({g,v:g[0].approxValue,x:xOf(g[0].approxValue,d)})).sort((a,b)=>a.x-b.x);
      const laneLastX=[-Infinity,-Infinity,-Infinity,-Infinity];
      positioned.forEach(({g,v,x})=>{ let lane=laneLastX.findIndex(last=>x-last>105); if(lane<0) lane=laneLastX.indexOf(Math.min(...laneLastX)); laneLastX[lane]=x; const c=E('circle',{cx:x,cy:d.y,r:8,fill:accent,stroke:card,'stroke-width':2,'data-point-id':g[0].id,style:'cursor:grab'}); svg.append(c); const label=E('text',{x,y:d.y-38-lane*28,fill:text,'font-size':12,'font-weight':700,'text-anchor':'middle','pointer-events':'none'}); label.textContent=g.map(p=>p.displayLabel).join(' = '); svg.append(label); const ap=E('text',{x,y:d.y-23-lane*28,fill:muted,'font-size':10,'text-anchor':'middle','pointer-events':'none'}); ap.textContent=`${tr('approx')} ${formatApprox(v)}`; svg.append(ap); });
      const sorted=positioned.map(x=>x.g[0]); order.textContent=sorted.length?sorted.map(p=>groups.get(p.equivalenceGroupId).map(q=>q.displayLabel).join(' = ')).join(' < '):tr('empty');
    }
    ...
    root.querySelectorAll('.mh-real-nline__interval input, .mh-rnl-interval-enabled').forEach(el=>el.addEventListener('input',draw));
    svg.addEventListener('pointerdown',e=>{ const target=e.target.closest('[data-point-id]'); if(!target) return; state.dragId=Number(target.getAttribute('data-point-id')); svg.setPointerCapture?.(e.pointerId); });
    ...
    (options.initialPoints||[]).forEach(addRaw); draw();
