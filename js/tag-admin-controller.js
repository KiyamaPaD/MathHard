...
export function createTagAdminController({ host, supabase, getCatalog=()=>({}), onChanged=()=>{} }={}){
    ...
    host.innerHTML=`
      <div class="mh-tag-studio-grid">
        <section class="mh-admin-card"><h3>Tag-uri normalizate</h3><p class="legend">Ordine și denumiri globale.</p>
          <form id="mhTagForm" class="mh-tag-form"><input id="mhTagId" placeholder="id-tag" required maxlength="100"><input id="mhTagRo" placeholder="Etichetă RO" required><input id="mhTagEn" placeholder="Label EN" required><label><input id="mhTagActive" type="checkbox" checked> Activ</label><button class="btn small" type="submit">Salvează</button><button class="btn small" type="button" data-tag-new>Tag nou</button></form>
          <div class="mh-tag-list">${catalog.tags.map((tag,i)=>`<article data-tag-id="${esc(tag.id)}"><div><strong>#${esc(tag.label_ro)}</strong><small>${esc(tag.id)} · ${tag.active?"activ":"inactiv"}</small></div><div><button class="btn small" type="button" data-tag-up ${i?"":"disabled"}>↑</button><button class="btn small" type="button" data-tag-down ${i<catalog.tags.length-1?"":"disabled"}>↓</button><button class="btn small" type="button" data-tag-edit>Editează</button><button class="btn small" type="button" data-tag-delete>Șterge</button></div></article>`).join("")||'<p class="legend">Niciun tag.</p>'}</div>
        </section>
  ...
  function bind(){
    ...
    form?.addEventListener("submit",async(e)=>{e.preventDefault(); try{await saveTag(supabase,{id:host.querySelector("#mhTagId").value,label_ro:host.querySelector("#mhTagRo").value,label_en:host.querySelector("#mhTagEn").value,active:host.querySelector("#mhTagActive").checked}); await refresh(); await onChanged();}catch(err){alert(err.message||err);}});
    host.querySelector("[data-tag-new]")?.addEventListener("click",()=>{const id=host.querySelector("#mhTagId");id.readOnly=false;id.value="";host.querySelector("#mhTagRo").value="";host.querySelector("#mhTagEn").value="";host.querySelector("#mhTagActive").checked=true;id.focus();});
    host.querySelectorAll("[data-tag-id]").forEach((row)=>{
      ...
      row.querySelector("[data-tag-edit]")?.addEventListener("click",()=>{host.querySelector("#mhTagId").value=tag.id;host.querySelector("#mhTagId").readOnly=true;host.querySelector("#mhTagRo").value=tag.label_ro;host.querySelector("#mhTagEn").value=tag.label_en;host.querySelector("#mhTagActive").checked=tag.active;});
    ...
    function paintMapping(){
      ...
      mapping.innerHTML=catalog.tags.map((tag)=>`<label><input type="checkbox" value="${esc(tag.id)}" ${active.has(tag.id)?"checked":""}> #${esc(tag.label_ro)}</label>`).join(""); save.disabled=false;
    }
