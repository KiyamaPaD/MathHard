export const CHAPTER_TRANSLATIONS = {
  "numere naturale": {
    ro: "Numere Naturale",
    en: "Natural Numbers"
  },
...
export function normalizeChapterKey(chapter) {
...
export function getCurrentLangSafe() {
  const lang =
    globalThis.window?.LANG ||
...
export function getChapterTranslation(chapter) {
...
export function getChapterLabel(chapter, lang = getCurrentLangSafe()) {
...
export function normalizeExamItem(item = {}, index = 0){
...
export function getTagTranslation(tag) {
...
export function getTagLabel(tag, lang = getCurrentLangSafe()) {
  const translation = getTagTranslation(tag);
  return lang === "en" ? translation.en : translation.ro;
}

export function getTagSearchBlob(tags = []) {
  return tags.map((tag) => {
    const translation = getTagTranslation(tag);
    return `${tag} ${translation.ro} ${translation.en}`;
  }).join(" ");
}
