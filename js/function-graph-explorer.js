...
function mountRepresentationLab(host) {
  ...
  function render() {
    ...
    graphCard.innerHTML = `<h3>${en ? "Graph" : "GRAFIC"}</h3>${graphSvg({ mode, activeInputs, selectedInput })}<div class="mh-representation-point-readout"><span>x = ${selectedInput}</span><span>f(${selectedInput}) = ${fx(selectedInput)}</span><span>\\[(${selectedInput},${fx(selectedInput)})\\in G_f\\]</span></div>`;
