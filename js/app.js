...
import { createSecureProblemController } from "./secure-problem-controller.js?v=1531";
import {
  getChapterLabel,
  getCurrentLangSafe,
  getTagLabel,
...
${details}`);
  ...
  async function mountContentAuthoringController({ reportError = false } = {}) {
    ...
    try {
      ...
      import("./admin-learning-polish-authoring.js?v=157").then(({mountAdminLearningPolishAuthoring})=>mountAdminLearningPolishAuthoring({form,language:LANG})).catch(()=>{});
 ...
 function mhUpdateHeaderStaticTexts(){
  ...
  function openViewer(item, forcedType = ""){
    ...
    if(isProblem){
      ...
      if(content.querySelector("[data-mh-function-graph-reader],[data-mh-function-zero-touch]")) import("./function-graph-reader.js?v=1581").then(({mountFunctionGraphReader})=>mountFunctionGraphReader(content)).catch((error)=>console.error("Function graph reader failed:",error));
      import("./learning-polish-controller.js?v=1581").then(({mountLessonPolish})=>{lessonPolishCleanup=mountLessonPolish({root:content,viewer:content,lesson:item,language:LANG});}).catch((error)=>console.error("Lesson polish failed:",error));
