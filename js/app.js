 ...
 function mhUpdateHeaderStaticTexts(){
  ...
  function openViewer(item, forcedType = ""){
    ...
    if(isProblem){
      ...
      if(content.querySelector("[data-mh-function-graph-reader],[data-mh-function-zero-touch]")) import("./function-graph-reader.js?v=158").then(({mountFunctionGraphReader})=>mountFunctionGraphReader(content)).catch((error)=>console.error("Function graph reader failed:",error));
      import("./learning-polish-controller.js?v=1572").then(({mountLessonPolish})=>{lessonPolishCleanup=mountLessonPolish({root:content,viewer:content,lesson:item,language:LANG});}).catch((error)=>console.error("Lesson polish failed:",error));
